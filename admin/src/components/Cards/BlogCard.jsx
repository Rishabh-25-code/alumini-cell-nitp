    ...
    return (
        <Link data-aos="fade-up" to={`/${type === "experience" ? "experience" : "blog"}/${data.$id}`}>
            <div className='w-[23rem] p-4 border-2 hover:border-gray-800  border-gray-900 rounded-2xl mt-5'>
                ...
                <h3 className='text-xl font-semibold pl-2 text-white'>{data.title.length > 48 ? data.title.substring(0, 48) + "..." : data.title}</h3>
                <div className='bg-gray-800 rounded-lg p-3 flex justify-between mt-4'>
                    <div>
                        <p className='text-gray-400 text-sm'>Date</p>
                        <p className='text-medium text-sm text-white'>{new Intl.DateTimeFormat('en-AU').format(new Date(data.$createdAt))}</p>
                    </div>
